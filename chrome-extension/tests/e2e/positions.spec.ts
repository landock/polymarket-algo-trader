...
test("recovers after retrying positions fetch", async () => {
  await clearExtensionStorage(context);
  await setE2EOverrides(context, {
    walletAddresses: {
      eoaAddress: "0x1111111111111111111111111111111111111111",
    ...
    positionsError: "Temporary positions error",
  });
  ...
  await page.locator("[data-cy=positions-error] button").click();
  await expect(page.locator("[data-cy=positions-list]")).toBeVisible();
  await expect(page.getByText("Retry Position")).toBeVisible();
});
